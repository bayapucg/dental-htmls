...
class Bookappointment extends CI_Controller {
	...
	{	
		if(!$this->session->userdata('user_details'))
		{
			...
			$this->load->view('html/book-appointment',$data);
	...
	$post=$this->input->post();
		    $addcontact=array(
				...
				$save=$this->Home_model->save_bookappointment_details($addcontact);
				if(count($save)>0){
						...
						$this->email->subject('Book Appointment- Request');
						$msg='Name:'.$post['name'].'<br> Email :'.$post['email'].'<br>  Phone  number :'.$post['phone_number'].'<br>  Age :'.$post['age'].'<br> Address :'.$post['address'].'<br> Services :'.$post['services'];
						$this->email->message($msg);
						$this->email->send();
					//echo'<pre>';print_r($msg);exit;

						//echo "test";exit;
						$this->session->set_flashdata('success',"Book Appointment was successfully sent.");
	...
	{	
		if($this->session->userdata('user_details'))
		{
			...
			$data['bookappointment_list']=$this->Admin_model->get_bookappointment_list();
			
			$this->load->view('admin/header',$data);
			$this->load->view('admin/sidebar');
			$this->load->view('admin/bookappointment-list',$data);
